  ...
  <div class="wrapper">
    <div class="header">
      ...
      <div class="item right">
        ...
        <img src="./icon/skin_header_icon_single.png" class="person" @click="$router.push(`/chat_one/${dataList.user_id}/set`)">
      </div>
    </div>
          ...
          <li v-for="item in dataList.message" :class="{
            ...
            <template v-if="item.type=='message'">
              <img :src="item.content.faceUrl" :class="{
                       'me':item.content.from=='me',
                       'other':item.content.from=='other'}" @click="$router.push(`/chat_one/${item.content.user_id}/profile`)">
            ...
            <template v-else-if="item.type=='voice'">
              <img :src="item.content.faceUrl" :class="{
                       'me':item.content.from=='me',
                       'other':item.content.from=='other'}" @click="$router.push(`/chat_one/${item.content.user_id}/profile`)">
              <p class="message"><img v-if="item.content.from=='me'" src="./icon/voice_me.png" @click="recPlay(item.content.message)">
                <img v-else src="./icon/voice_other.png" @click="recPlay(item.content.message)">
              </p>
              <audio ref="record" :src="item.content.message"></audio>
            </template>
            <template v-else>
              {{item.content}}
            </template>
    ...
    <div class="footer">
      <div class="writeMessage">
        ...
        <button style="width: 60px" @click="startRecord">开始</button>
        <button style="width: 60px" @click="endRecord">结束</button>
      </div>
...
import * as recorder from "../../../Recorder-master/recorder"
...
export default {
  ...
  methods: {
    ...
    endRecord() {
      let _this = this;
      this.rec.stop(function(blob) { //到达指定条件停止录音，拿到blob对象想干嘛就干嘛：立即播放、上传
        _this.blob = URL.createObjectURL(blob);
        // window.localStorage.setItem(_this.blob, JSON.stringify(blob))
    ...
    // 播放语音
    recPlay(src) {
      let audioArr = this.$refs.record,
        blob;
      for (let i in audioArr) {
        if (audioArr[i].src == src) {
          // blob = JSON.parse(window.localStorage.getItem(src))
          // var binaryData = [];
          // binaryData.push(blob);
          // let b = new Blob(binaryData, {type: "application/zip"})
          audioArr[i].play()
        }
      }
    },
    ...
    //发送消息
    async sendMessage() {
      let type = this.writeMessage === " " ? "voice" : 'message',
        msg = type === "voice" ? this.blob : this.writeMessage;

      this.$refs.message.focus()
      this.btnInfo = '发送中'

      this.sendBySocket(msg, type) //先通过socket发送消息
    ...
    updateBySocket() {
      ...
      socket.on('receivePrivateMessage', (data) => {
        console.log(data)
        if (data.type == "voice") {
          // this.$refs.record.src = data.message;
